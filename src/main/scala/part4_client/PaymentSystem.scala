...
object PaymentSystem extends App with PaymentJsonProtocol with SprayJsonSupport {
  ...
  // micro service for payments
  implicit val system = ActorSystem("PaymentSystem")
  implicit val materializer = ActorMaterializer()
  import system.dispatcher

  val paymentValidatorActor = system.actorOf(Props[PaymentValidator], "paymentValidator")
    ...
    path("api" / "payments") {
      post {
        entity(as[PaymentRequest]) { paymentRequest =>
          val validationResponseFuture = (paymentValidatorActor ? paymentRequest).map {
