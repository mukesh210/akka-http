...
object WebsocketsDemo extends App {
  ...
  val html =
    """
      ...
      |    <head>
      |        <script>
      |            var exampleSocket = new WebSocket("ws://localhost:8081/greeter")
  ...
  def websocketFlow: Flow[Message, Message, Any] = Flow[Message].map {
    ...
    case bm: BinaryMessage =>
      bm.dataStream.runWith(Sink.ignore)  // important otherwise will lead to memory leak
  ...
  Http().bindAndHandle(websocketRoute, "localhost", 8081)

  // serve pushing data to client
  case class SocialPost(owner: String, content: String)

  val socialFeed = Source(
    List(
      SocialPost("Martin", "Scala 3 has been announced"),
      SocialPost("Daniel", "A new RTJ course is available"),
